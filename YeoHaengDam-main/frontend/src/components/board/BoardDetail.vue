...
const fetchComments = async () => {
  try {
    const response = await axios.get(
      `https://api_yeohaengdam.ttalkak.com/api/comment/${props.item.articleId}`
    );
...
const postComment = async () => {
  ...
  try {
    await axios.post(
      'https://api_yeohaengdam.ttalkak.com/api/comment',
      {
  ...
  if (userInfo.value.nickname === props.item.nickname) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://api_yeohaengdam.ttalkak.com/api/articles/${props.item.articleId}`,
            {
              headers: {
